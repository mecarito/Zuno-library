...
<template>
  <div class="h-search-h flex flex-row items-center w-full border-[1px]
    ...
    <div class=" flex flex-row items-center w-full">
      ...
      <input type="text" placeholder="Find the book you like..."
        class="mx-4 h-full outline-none placeholder-dark px-4 w-full" v-model="search"
        @input="$emit('searchTerm', search)" />
    </div>
    <button class="w-btn-w h-btn-h bg-green text-white flex flex-row 
        justify-center items-center rounded-lg space-x-4" @click="$emit('searchTerm', search)">
